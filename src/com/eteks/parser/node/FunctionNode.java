...
import com.eteks.jeks.JeksExpressionSyntax.JeksFunctionDeref;
import com.eteks.jeks.JeksExpressionSyntax.JeksFunctionIf;
...
public class FunctionNode implements ParameterizedNode
{
     ...
     * 
     * @param function
     *            a previously compiled function or any instance of a Java
     *            written class implementing the <code>Function</code>
     *            interface.
     * @param recursiveCall
     *            <code>true</code> if this is a recursive call.
     */
    ...
    /**
     * Returns the expression at index <code>index</code> that will be passed as
     ...
     * of <code>interpreter</code> with the instance of <code>Function</code> of
     * this node and the computed value of its parameters as parameters of
     * <code>getFunctionValue ()</code>.
     * 
     * @param interpreter
     *            runtime interpreter.
     * @param parametersValue
     *            the value of parameters passed to compute a compiled function.
     * @return the computed value of this function. The type of the returned
     *         value depends on the implementation of the interpreter.
            ...
            Object[] parametersValue)
    {
        ...
        if(function instanceof LazyFunction)
        {
            ...
            for (int i = 0; i < parametersExpression.size(); i++)
            {
                functionParameters[i] =
                        new CallByNameArgument(parametersValue, getParameter(i));
            }
        ...
        else
        {
            ...
            {
                functionParameters[i] = 
                        getParameter(i).computeExpression(interpreter, parametersValue);
            }
        }


        if (function instanceof CompiledFunction
                && ((CompiledFunction) function).isTypeFunction())
        {
            if (interpreter instanceof TypeIntepreter)
            {
                JeksInterpreter jeksInterpreter = (JeksInterpreter) interpreter;

                // TODO: FUNCTION NODE SHOULDN'T KNOW ABOUT USER CODE
                if (function.getName().endsWith("_unprotected"))
                {
            ...
            throw new IllegalArgumentException(
                    "Unable to evalue function, incorrect Interpreter type.");
        }
        return interpreter.getFunctionValue(function, functionParameters,
                isRecursiveCall());
    }
    ...
    /**
     * Returns the double value of this function call. If the called function
     * isn't an instance of <code>CompiledFunction</code>, an instance of
     * <code>DoubleInterpreter</code> is passed to the
     * <code>computeFunction ()</code> of the function to compute its value.
     * 
     * @param interpreter
     *            runtime interpreter.
     * @param parametersValue
    ...
    public double computeExpression(double[] parametersValue)
    {
        if (function instanceof CompiledFunction)
        {
            double[] functionParameters = new double[parametersExpression
                    .size()];
            for (int i = 0; i < parametersExpression.size(); i++)
                functionParameters[i] = getParameter(i).computeExpression(
                        parametersValue);

            return ((CompiledFunction) function)
                    .computeFunction(functionParameters);
        } else
        {
            // As Function interface declares only the method
            // public Object computeFunction (Interpreter interpreter, Object []
            // parametersValue)
            // the value is computed with the DoubleInterpreter interpreter
            // class
            Double[] functionParameters = new Double[parametersExpression
                    .size()];
            for (int i = 0; i < parametersExpression.size(); i++)
                functionParameters[i] = new Double(getParameter(i)
                        .computeExpression(parametersValue));

            if (doubleInterpreter == null)
                doubleInterpreter = new DoubleInterpreter();

            Object result = function.computeFunction(doubleInterpreter,
                    functionParameters);
